...
export async function getPlans(filters?: {
    ...
    const plans: Plan[] = data.map((p: any) => ({
        ...
        validityType: 'year',
        validityValue: p.duration_days ? Math.round(p.duration_days / 364) : 1,
...
export async function getPlan(id: string) {
    ...
    const plan: Plan = {
        ...
        validityType: 'year',
        validityValue: data.duration_days ? Math.round(data.duration_days / 364) : 1,
...
export async function createPlan(data: Partial<Plan>) {
    ...
    const { error } = await supabase.from('plans').insert({
        ...
        allowed_services: data.allowed_services || [],
        duration_days: (data.validityValue || 1) * (data.validityType === 'month' ? 30 : 364),
        type: data.type,
        status: planStatus,
        slug: data.slug,
...
export async function updatePlan(id: string, data: Partial<Plan>) {
    ...
    const { error } = await supabase.from('plans').update({
        ...
        category_ids: data.categoryIds || [],
        allowed_services: data.allowed_services || [],
        duration_days: data.validityType === 'year' ? (data.validityValue || 1) * 364 : data.validityValue ? data.validityValue * 30 : 364,
        status: planStatus,
        slug: data.slug,
        is_active: planStatus === 'active',
        is_featured: data.isFeatured,
